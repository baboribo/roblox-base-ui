...
Base UI 기반 React 컴포넌트와 디자인 토큰입니다. 컴포넌트 파일을 프로젝트에 복사해 사용합니다. 문서는 Fumadocs, 상태 비교는 Storybook으로 제공합니다.

Roblox의 공개 CSS와 화면을 참고한 독립 구현입니다. 공식 패키지나 내부 디자인 시스템 전체의 복제본은 아닙니다.

## 디자인 원칙과 작업 기준

[디자인 원칙](docs/content/design-principles.mdx)을 설계·구현·리뷰의 기준으로 사용합니다. 안정성을 먼저 확보하고 명확성을 함께 충족하며, 모션으로 둘을 지원합니다. 기존 구현이 모두 충족했다는 선언은 아닙니다.

변경 전에는 원칙과 대상 컴포넌트 문서를 읽습니다. AI 작업 절차는 [AGENTS.md](AGENTS.md), 구조와 검증 명령은 [개발 안내](docs/content/development.mdx)를 따릅니다.
