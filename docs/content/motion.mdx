...
움직임은 상태와 공간의 변화를 설명할 때 사용합니다. 같은 역할에는 공통 규칙을 적용하되, 거리·크기·반복 사용 빈도에 맞게 조정합니다. 모든 컴포넌트에 같은 시간이나 효과를 적용하지 않습니다. [디자인 원칙](/docs/design-principles)에 따라 안정성과 명확성을 먼저 충족합니다.

아래 표는 현재 구현 값입니다. 새로운 동작에 그대로 복사하는 기준은 아닙니다. 변경할 때는 연속 조작·중단·반전, 퇴장 중 조작 가능 여부, 모션 감소 설정을 함께 확인합니다.
